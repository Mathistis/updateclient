...
int32_t MbedApplication::checkApplication() {
  ...
  if (m_applicationHeader.firmwareSize > 0) {
    ...
    int err = mbedtls_sha256_starts_ret(&mbedtls_ctx, 0);
    if (err != 0){tr_err("Can't start mbedtls_sha256 : %d", err);}
    ...
    while (remaining > 0) {
      ...
      err = m_flashUpdater.read(m_buffer, m_applicationAddress + (m_applicationHeader.firmwareSize - remaining), readSize);
      if (err != 0) {
      ...
      // update hash
      err = mbedtls_sha256_update_ret(&mbedtls_ctx, m_buffer, readSize);
      if (err != 0){tr_err("Error updating ctx mbed tls sha256 : %d", err);}
      if (once*2){
    ...
    if (err != 0){tr_err("Error finishing hash mbed tls sha256 : %d", err);}
    mbedtls_sha256_free(&mbedtls_ctx);

    // compare calculated hash with hash from header
    printf("COMPARE : \n");
    for (int i = 0; i < SIZEOF_SHA256; i++){
        printf("%x ", *(SHA+i));

    }
    printf("\n");
    for (int i = 0; i < SIZEOF_SHA256; i++){
        printf("%x ", *(m_applicationHeader.hash+i));
...
void MbedApplication::compareTo(MbedApplication& otherApplication) {
  ...
  if (result != UC_ERR_NONE) {
    tr_error(" Other application not valid : %d", result);
    return;
  }
  ...
  if (0 == err) {    
    ...
    switch (m_applicationHeader.headerVersion) {
      case HEADER_VERSION_V2: {
        // TODO
        // CHeck magic, if success read header and use parseInternalHeaderV2
        
        if(m_applicationHeader.magic == HEADER_MAGIC_V2) {
            //tr_debug("Magic number correct (0x5a51b3d4)\n");
            result = UC_ERR_NONE;
            uint8_t read_buffer[HEADER_SIZE_V2] = {0};
            int ret = m_flashUpdater.read(read_buffer, m_applicationHeaderAddress,HEADER_SIZE_V2);
            if(ret == 0){
                result = parseInternalHeaderV2(read_buffer);
            }
        }
        else{
            //tr_debug("Magic number invalid \n");
        }
      }   
      break;
