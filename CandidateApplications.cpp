...
uint32_t CandidateApplications::getSlotForCandidate() { 
  // TODO
  uint32_t res = 0;
  tr_debug("Get slot for candidates : \n");

  // Check validity
  for (uint32_t i = 0; i < MBED_CONF_UPDATE_CLIENT_STORAGE_LOCATIONS; i++){
    if(!m_candidateApplicationArray[i]->isValid()){
        tr_debug("Slot %d invalid was chosen\r\n",i);
        return i;
    }
  }
  // If both valid, check for the oldest version
    if (m_candidateApplicationArray[0]->isNewerThan(
          *m_candidateApplicationArray[1])) {
    tr_debug("Slot %d is the oldest and was chosen\r\n", 1);
    res = 1;
  } else {
    tr_debug("Slot %d is the oldest and was chosen\r\n",0) ;
    res = 0;
...
//   for (uint32_t i = 0; i < MBED_CONF_UPDATE_CLIENT_STORAGE_LOCATIONS; i++){
//       if(m_candidateApplicationArray[i]->isNewerThan(*m_candidateApplicationArray[res])){
//           tr_debug("Slot %d is the oldest and was chosen\r\n",i);
//           res = i;
//       }
//   }
...
int32_t CandidateApplications::installApplication(uint32_t slotIndex, uint32_t destHeaderAddress) {  
  ...
  while (nbrOfBytes < copySize) {
    ...
    memset(writePageBuffer.get(),0,sizeof(char)*pageSize); //Reinitialize writePage bufer
    result = m_flashUpdater.readPage(pageSize,writePageBuffer.get(),sourceAddr);
    if(result != UC_ERR_NONE){
        tr_error("Unable to read candidate application at slot %d (addr : 0x%08x)",slotIndex,sourceAddr);
        return result;
    }
    //writePageBuffer = readPageBuffer.get();
    ...
    if(result != UC_ERR_NONE){
        tr_error("Unable to write candidate app at slot %d (addr : 0x%08x)",slotIndex,destAddr);
        return result;
    }
    // update progress
    nbrOfBytes += pageSize;    
